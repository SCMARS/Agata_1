...
class StageController:
    ...
    def get_user_stage(self, user_id: str, message_count: int) -> int:
        ...
        Stage 1: 1-8 сообщений
        Stage 2: 9-16 сообщений  
        Stage 3: 17+ сообщений
        """
        current_time = datetime.now().strftime("%H:%M:%S")
        
        # 🔥 ПРОСТАЯ ЛОГИКА: только счетчик сообщений
        if message_count <= 8: 
            stage = 1
    ...
    def get_stage_question(self, user_id: str, stage: int) -> str:
        ...
        logger.info(f"🔍 [GET_QUESTION] {user_id}: Ищу вопрос для стейджа {stage}")
        
        # Получаем все доступные вопросы для стейджа
        stage_rules = self.stage_rules.get(stage, {})
        themes = stage_rules.get("themes", {})
        ...
        logger.info(f"🔍 [GET_QUESTION] {user_id}: Задано вопросов: {len(asked_questions)}")
        logger.info(f"🔍 [GET_QUESTION] {user_id}: Доступно вопросов: {len(all_questions)}")
        
        # Ищем незаданные вопросы
        unasked_questions = [q for q in all_questions if q not in asked_questions]
        
        if unasked_questions:
            # Выбираем первый незаданный вопрос
            candidate = unasked_questions[0]
            logger.info(f"✅ [GET_QUESTION] {user_id}: Выбран незаданный вопрос: '{candidate}'")
        else:
            # Если все вопросы заданы, выбираем случайный из всех
            import random
            candidate = random.choice(all_questions)
            logger.info(f"🔄 [GET_QUESTION] {user_id}: Все вопросы заданы, выбираем случайный: '{candidate}'")
...
"""
    ...
    def analyze_user_response_and_close_slots(self, user_id: str, user_message: str, stage_number: int):
        ...
        # 🔥 РАСШИРЕННЫЙ маппинг ключевых слов для автоматического закрытия слотов
        slot_keywords = {
            # Знакомство
            "Как тебя зовут?": ["зовут", "имя", "меня", "зову", "глеб", "алексей", "андрей", "сергей", "дмитрий", "максим", "владимир", "иван", "николай", "михаил", "павел"],
            "Сколько тебе лет?": ["лет", "возраст", "года", "годиков", "мне", "исполн", "родился", "25", "30", "35", "40"],
            "Что привело тебя сюда?": ["пришел", "зашел", "заинтерес", "понравился", "профиль", "фото", "привело", "нашел", "увидел"],
            
            # Жительство
            "Откуда ты?": ["откуда", "из", "живу", "город", "страна", "москва", "киев", "минск", "санкт", "питер", "екатеринбург", "украина", "россия", "беларусь"],
            "Как давно там живёшь?": ["давно", "живу", "лет", "года", "переехал", "родился", "с детства", "5 лет", "10 лет", "всю жизнь"],
            "Почему именно этот город?": ["почему", "выбрал", "нравится", "люблю", "приехал", "работа", "учеба", "семья", "друзья"],
            "Что тебе там больше всего нравится?": ["нравится", "люблю", "красиво", "атмосфера", "люди", "места", "парки", "архитектура", "культура"],
            "Какие места посоветуешь посетить?": ["места", "посоветуешь", "посетить", "достопримечательности", "музеи", "парки", "кафе", "рестораны", "театры"],
            
            # Работа
            "Кем работаешь?": ["работаю", "работа", "профессия", "дизайнер", "программист", "менеджер", "инженер", "врач", "учитель", "разработчик", "it"],
            "Давно этим занимаешься?": ["давно", "занимаюсь", "опыт", "лет", "года", "работаю", "уже", "недавно", "давно"],
            "Что нравится больше всего?": ["нравится", "люблю", "интересно", "больше всего", "лучше всего", "увлекательно", "креативно"],
            "Сколько удается зарабатывать, если не секрет?": ["зарабатываю", "зарплата", "доход", "деньги", "рублей", "долларов", "евро", "тысяч", "миллионов"],
            "Легко ли совмещать с личной жизнью?": ["совмещать", "личная жизнь", "семья", "время", "свободное время", "баланс", "работа-жизнь"],
            "Как отношения с коллегами?": ["коллеги", "отношения", "команда", "коллектив", "работают", "хорошо", "дружно", "поддерживают"],
            
            # Хобби
            "Чем любишь заниматься в свободное время?": ["свободное время", "люблю", "занимаюсь", "хобби", "отдыхаю", "спорт", "кино", "книги", "музыка", "путешествия"],
            "У тебя активный отдых или спокойный?": ["активный", "спокойный", "отдых", "спорт", "гуляю", "дома", "на природе", "в городе"],
            "Как относишься к спорту?": ["спорт", "тренировки", "фитнес", "бег", "зал", "отношусь", "занимаюсь", "люблю", "не люблю"],
            "Любишь готовить?": ["готовить", "готовлю", "кулинария", "кухня", "блюда", "рецепты", "повар", "еда"],
            "Какие фильмы или книги предпочитаешь?": ["фильмы", "книги", "предпочитаю", "люблю", "читаю", "смотрю", "жанр", "фантастика", "драма", "комедия"],
            
            # Личное/Флирт
            "У тебя есть девушка?": ["девушка", "жена", "женат", "одинок", "встречаюсь", "отношения", "семья", "свободен"],
            "Что для тебя важно в отношениях?": ["отношения", "важно", "доверие", "понимание", "любовь", "верность", "поддержка"],
            "Как относишься к сюрпризам?": ["сюрпризы", "сюрприз", "подарки", "неожиданность", "люблю", "не люблю", "приятно"],
            
            # Stage 2 - Цели/мечты
            "Какие у тебя планы на будущее?": ["планы", "будущее", "цели", "мечты", "хочу", "планирую", "стремись"],
            "Есть ли мечта, которую хочешь осуществить в ближайшее время?": ["мечта", "осуществить", "ближайшее время", "хочу", "цель", "планы"],
            "Чего тебе сейчас не хватает для счастья?": ["счастья", "не хватает", "хочу", "нужно", "мечтаю", "стремлюсь"],
            "Если бы у тебя было больше свободных средств — куда бы потратил?": ["средства", "деньги", "потратил", "купил", "инвестировал", "путешествия"],
            
            # Stage 2 - Автомобиль
            "У тебя есть машина?": ["машина", "автомобиль", "есть", "нет", "вожу", "водительские права"],
            "Давно за рулём?": ["за рулем", "вожу", "давно", "опыт", "водительские права", "машина"],
            "Какая машина тебе нравится больше всего?": ["машина", "нравится", "марка", "модель", "bmw", "mercedes", "audi", "toyota"],
            "Какая твоя машина мечты?": ["машина мечты", "мечтаю", "хочу", "идеальная", "суперкар", "спорткар"],
            
            # Stage 2 - Семья
            "Часто видишься с родителями?": ["родители", "видишься", "часто", "редко", "мама", "папа", "семья"],
            "Есть братья/сёстры?": ["братья", "сестры", "брат", "сестра", "семья", "родственники", "есть", "нет"],
            "Какие воспоминания из детства самые яркие?": ["детство", "воспоминания", "яркие", "помню", "было", "играл", "учился"],
            "Ближе к маме или папе?": ["мама", "папа", "ближе", "отношения", "семья", "люблю", "понимаю"],
            
            # Stage 2 - Флирт
            "Какие качества ты ценишь в девушках?": ["качества", "девушках", "ценю", "важно", "характер", "внешность", "ум"],
            "Как представляешь идеальные отношения?": ["идеальные отношения", "представляю", "мечтаю", "хочу", "любовь", "семья"],
            "Был ли женат?": ["женат", "был", "разведен", "брак", "семья", "жена", "дети"],
            "Есть ли дети?": ["дети", "ребенок", "сын", "дочь", "есть", "нет", "планирую", "хочу"]
        }
        ...
        if slots_closed > 0:
            logger.info(f"✅ [SLOT_ANALYSIS] {user_id}: Закрыто {slots_closed} слотов")
        else:
            logger.info(f"ℹ️ [SLOT_ANALYSIS] {user_id}: Слоты не закрыты (не найдены ключевые слова)")
