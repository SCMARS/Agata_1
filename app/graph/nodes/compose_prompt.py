...
Вопросы разрешены только каждое 2-е сообщение!"""    
    ...
    def _create_behavioral_instructions(self, behavioral_analysis: Dict[str, Any]) -> str:
        ...
        instructions.append("🚨 КРИТИЧЕСКИ ВАЖНО: НИКОГДА не начинай ответы одинаково!")
        instructions.append("СТРОГО ИЗБЕГАЙ: 'Классно, что...', 'Это здорово...', 'Приятно слышать...', 'Киев...', 'Вижу, ты...'")
        instructions.append("ОБЯЗАТЕЛЬНО начинай РАЗНО: 'Круто!', 'Ого!', 'Вау!', 'Серьезно?', 'Ага', 'Точно', 'Хм', 'Да?', 'Правда?'")
        instructions.append("Используй междометия: 'Ох', 'Блин', 'Ух ты', 'Вот это да!', 'Не ожидала!', 'Ого-го!'")
        instructions.append("Каждый ответ должен начинаться с РАЗНОГО слова - как у живого человека!")
        instructions.append("Проверяй: если предыдущий ответ начинался с 'Круто!', следующий НЕ должен начинаться с 'Круто!'!")
